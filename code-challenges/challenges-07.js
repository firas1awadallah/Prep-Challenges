...
const cvFormatter = (arr) => {
    let newArray =[];
    for (let key in arr) {
        if(arr[key].yearsOfExperience > 1 && arr[key].lastName !== null) {
            
               newArray.push({ fullName : arr[key].firstName +" "+ arr[key].lastName,tech :arr[key].tech})
                 
     }
         else if(arr[key].yearsOfExperience > 1 && arr[key].lastName == null) {
           newArray.push({fullName : arr[key].firstName , tech : arr[key].tech}); 
