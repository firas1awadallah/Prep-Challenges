...
const cvsFiltration = (arr) => {
 let filterCv = arr.filter(cv => {
    return cv.yearsOfExperience >4 && cv.tech =="JS";
 });
 let filterCvs = filterCv.map(cvs => {
    let fullName = [cvs.firstName,cvs.LastName].filter(name => name!==null).join(" ");
return{fullName: fullName,tech: cvs.tech};
})
 return  filterCvs;
}
// -------------------------------------------------------------------------------------------------------
...
const vowelsFiltration = (arr) => {
    ...
    return arr.filter(word => {
        return !vowels.some(vowel =>{
            return word.toLowerCase().includes(vowel);
        });
        })
      } 
// -------------------------------------------------------------------------------------------------------
